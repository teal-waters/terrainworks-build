...
def bin_env(tmp_path):
    ...
    with (
        patch("terrainworks_build._bin_dir", return_value=tmp_path),
...
def test_deletes_and_redownloads_stale_binary(bin_env):
    ...
    with patch(
        "terrainworks_build.urllib.request.urlretrieve", side_effect=_fake_download
    ):
        result = tw.get_binary_path("bldgrds")
    assert result == str(bin_env / "bldgrds")
...
def test_download_failure_raises_file_not_found():
    with patch(
        "terrainworks_build.urllib.request.urlretrieve", side_effect=OSError("timeout")
    ):
...
def test_unknown_version_raises_when_no_binary(tmp_path):
    with (
        ...
        patch.object(tw, "__version__", "unknown"),
        patch(
            "terrainworks_build.urllib.request.urlretrieve",
            side_effect=OSError("no such release"),
        ),
    ):
